...
void SpectrumBarsRenderer::render(juce::Graphics& g, juce::Rectangle<int> b, const std::array<float, AudioAnalyzer::spectrumBins>& s, float pulse, const VisualRackState& rack)
{
    ...
    for (int i = 0; i < bars; ++i)
    {
        ...
        if (rack.fxBloom.load()) { g.setColour(col.withAlpha(0.2f + 0.2f * rack.bloomAmount.load())); g.fillRect(b.getCentreX() - (i + 1) * step, mid - barH - 3.0f, thickness, barH * 2.0f + 6.0f); g.fillRect(b.getCentreX() + i * step, mid - barH - 3.0f, thickness, barH * 2.0f + 6.0f); }
    ...
    g.setColour(juce::Colour::fromRGB(32, 218, 255).withAlpha(0.35f + pulse * 0.3f)); g.drawLine((float)b.getX(), mid, (float)b.getRight(), mid, 1.5f);
}
