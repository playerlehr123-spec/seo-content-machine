...
void SpectrumBarsRenderer::render(juce::Graphics& g, juce::Rectangle<int> b, const std::array<float, AudioAnalyzer::spectrumBins>& s, float pulse, const VisualRackState& rack)
{
    ...
    g.fillAll(juce::Colour::fromRGB(3, 6, 13));
    ...
    g.setColour(juce::Colour::fromRGB(7, 14, 26));
    g.fillRoundedRectangle(scope.expanded(8.0f), 18.0f);
    g.setColour(juce::Colour::fromRGB(32, 218, 255).withAlpha(0.07f + activity * 0.10f));
    g.fillRoundedRectangle(scope, 15.0f);
    g.setColour(juce::Colour::fromRGB(32, 218, 255).withAlpha(0.18f + activity * 0.18f));
    g.drawRoundedRectangle(scope, 15.0f, 1.2f);
