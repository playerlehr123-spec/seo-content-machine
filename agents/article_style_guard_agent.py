...
class ArticleStyleGuardAgent(BaseAgent):
    ...
    def run(self, input_data):
        ...
        banned_found = [p for p in banned_phrases if p in lowered]
        ...
        pricing_risks = [p for p in ["guaranteed revenue", "pricing starts at", "hard roi guarantee", "statistically proven"] if p in lowered]

        required_sections = ["quick answer", "why it matters", "practical examples", "how the tv sign helps", "common mistakes", "faq", "cta"]
        missing_items = [f"missing section: {s}" for s in required_sections if f"## {s}" not in lowered and f"# {s}" not in lowered]
        ...
        passes_structure = len([m for m in missing_items if "missing section" in m or "too many sections" in m]) == 0
        approved = passes_length and passes_structure and not banned_found and not wrong_terms and not pricing_risks
